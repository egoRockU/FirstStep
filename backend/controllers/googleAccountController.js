...
const createGoogleAccount = asyncHandler(async (req, res) => {
  ...
  const insertResult = await GoogleAccount.create({
    email,
    sub: subHash,
    uniqueString,
  });
...
const addProfile = asyncHandler(async (req, res) => {
  ...
  const updateResult = await GoogleAccount.findOneAndUpdate(
    { email },
    changeValues
  );
...
export { getAllGoogleAccounts, createGoogleAccount, loginGoogle, addProfile };
